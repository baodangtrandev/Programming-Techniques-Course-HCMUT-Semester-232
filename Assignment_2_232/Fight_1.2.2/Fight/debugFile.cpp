...
private:
    // TODO
    string moving_rule;
    inline static int next_char_index = 0;
...
string RobotS::str(){
        ...
        stringstream ss;
        ss << "RobotS[pos=" << pos << ";type=S;dist=" << dist << "]";
        return ss.str();
    }
////////////////////////////////////////////////////////////////////////
// class RobotW
...
string RobotW::str() {
        int dist = getDistance();
        stringstream ss;
        ss << "RobotW[pos=" << pos << ";type=W;dist=" << dist << "]";
        return ss.str();
    }
...
string RobotSW::str() {
        int dist = getDistance();
        stringstream ss;
        ss << "RobotSW[pos=" << pos << ";type=SW;dist=" << dist << "]";
        return ss.str();
    }
...
bool Sherlock::meet(Watson *watson)
{
    ...
    cout <<  "passing gift" << endl;
    ItemNode* temp = gift;
    while(temp!=nullptr){
        //BaseItem* item = temp->item;
        ItemNode* itemNode = temp;
        watson->getBag()->insert(itemNode->item);
        temp = temp->next;
        delete itemNode;
    }
    cout << "passing insert" << endl;
    int countWatson = watson->getBag()->deleteItem(EXCEMPTION_CARD);
    cout << "Watson delete" << endl;
...
{
    StudyPinkProgram program(inputFile);
    program.run(OUTPUT);
}
