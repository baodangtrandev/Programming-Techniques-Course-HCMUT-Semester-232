...
int tinh_tong_chu_so(int n) {
 ...
 Robot::Robot(RobotType type, int index, const Position& init_pos, Map* map)
        : MovingObject(index, init_pos, map, "Robot"), robot_type(type){
        ...
        for(int i = 0; i < count; i++){
            if(i != index){
                if(arr_mv_objs[i]->getCurrentPosition().isEqual(arr_mv_objs[index]->getCurrentPosition().getRow(),arr_mv_objs[index]->getCurrentPosition().getCol())){
                    if(arr_mv_objs[index]->getName()=="Sherlock"){
                        arr_mv_objs[index]->meet(arr_mv_objs[i]);
                        return true;
                    }else if(arr_mv_objs[index]->getName()=="Watson"){
                        arr_mv_objs[index]->meet(arr_mv_objs[i]);
                        return true;
                    }else if(arr_mv_objs[index]->getName()!="Sherlock"){
                        if(arr_mv_objs[i]->getName()=="Sherlock"){
                            arr_mv_objs[i]->meet(arr_mv_objs[index]);
                            return true;
